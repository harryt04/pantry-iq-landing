...
| File | Covers |
| --- | --- |
| [`critical-path.spec.ts`](../tests/critical-path.spec.ts) | Sign up → create location → import a CSV → resolve an item → commit → dashboard; transaction corpus batches 1–3 and purchase-order batches 1–2 through the authenticated `/import` route |
